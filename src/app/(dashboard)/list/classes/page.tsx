...
const ClassListPage = async ({
	...
	const columns = [
		...
		{
			header: 'Кол-во студентов/учеников',
			accessor: 'capacity',
	...
	const renderRow = (item: ClassList) => (
		...
		>
			...
			<td className='hidden sm:table-cell'>{extractGradeLevel(item.name)}</td>
			<td className='hidden md:table-cell'>
				{item.supervisor.surname +
					' ' +
					item.supervisor.name +
					' ' +
					item.supervisor.patronymic}
			</td>
			<td>
	...
	return (
		<div className='bg-white p-4 rounded-md flex-1 m-4 mt-0'>
			...
			<div className='flex items-center justify-between'>
				<h1 className='hidden md:block text-lg font-semibold'>Все группы</h1>
